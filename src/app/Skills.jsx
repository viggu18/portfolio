...
const Skills = () => {
  return (
    <div className="flex gap-2 flex-col" id="skills">
      <h1 className="text-xl">Skills</h1>
      <div className="flex flex-wrap gap-4 my-3 items-center justify-center">
        <svg
          viewBox="0 0 71 20"
          fill="none"
          role="img"
          className="icon-md w-[72px] h-5 text-default my-1 css-16w551c hover:scale-125"
          title="Expo"
        >
          <path
        ...
        {SkillsArr.map((item) => (
          <i
            className={`${item} colored mx-1 xl:text-[6rem] lg:text-[6rem] md:text-[5rem] sm:text-[4rem] xs:text-[4rem] hover:scale-125`}
            key={item}
          />
        ))}
      </div>
